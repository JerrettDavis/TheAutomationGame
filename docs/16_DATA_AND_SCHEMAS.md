...
S035 adds a separate schema-v3 facilitator observation beside that objective export. It records only an anonymous session ID, timestamp, `Human`/`SyntheticFixture` provenance, selected guidance mode, vocabulary cohort flag, independent boolean judgments for movement, interaction, bottleneck, reported/physical readiness, replay/proof, and pre-name Strategy expression, action-directed help, an optional stable blocker code, and critical issues with owner plus `Fixed`/`Backlog` disposition. Validation rejects missing identity/timestamp, unsupported versions, duplicate or unowned issues, and unknown enum values. Aggregation also rejects a directory/observation/completion session mismatch, disagreement between structured and objective guidance, conflicting ownership/disposition for a stable issue code, or completion evidence that does not prove onboarding, the exact full quest set, a passed reliability window, and an available report. Neither research DTO is a career save or replay input.
