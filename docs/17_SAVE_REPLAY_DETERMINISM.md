...
`AutomationCareerSaveStore` is the current versioned career envelope. It contains the first-shift replay, bounded two-station routing replay, vendor-comparison replay, and the player's immutable pattern-knowledge profile. Pattern evidence keeps stable source quest, scenario, industry, problem-signature, player-move, replay reference, and consequence fields; recognition, naming, and mastery conclusions retain the exact evidence ID that supports them rather than being inferred from client state. Schema 2 added those persisted conclusion citations. Schema 3 adds the S033 proposal-selection/trial journal and reconstructs its exact cost, service, ownership, and incident traces. Schema-1 careers migrate recognition to applied evidence; schema-2 and raw first-shift careers begin the optional vendor arc empty. Explicit semantic-ID converters reject malformed or null IDs, duplicate journals/evidence/conclusions fail validation, and atomic replacement remains the only durable write path.
