...
Integration validation covers atomic replacement, removal of the temporary file, JSON preservation of onboarding and early progression, midpoint continuation, and future-command effects. Simulation validation restores the completed live-window attempt, frozen shift-report evidence, and eight-quest progression from the replay journal, and proves that later sandbox ticks do not change career duration or the report. The native UI driver closes the process, relaunches it, exercises the protected New Career branch, and resumes the level-7 career with the passed 3/3 scorecard. God mode still exposes separate in-memory quick-save/restore through `F10` and `F11` for test setup.
